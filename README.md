...
**1. Login with Username (Auto-Signup)**
```
...
Query Parameters:
  q: search query (searched in username, firstName, lastName)
...
**11. Register Device Token (Push Notifications)**
```
...
{
  "callId": "call_id",
  "caller": { "id": "user_id", "username": "testuser", "isDeaf": true },
  "callee": { "id": "other_id", "username": "otheruser", "isDeaf": false },
  "state": "initiated",
  "createdAt": "2026-04-14T10:00:00Z"
}
```
...
Response: 200 OK
{
  "callId": "call_id",
  "transcript": [
    { "text": "Hello, how are you?", "speaker": "hearing", "timestamp": 1713094800000 }
  ]
}
```
...
**18. Add Message to Conversation**
```
...
Response: 200 OK
{
  ...
  "participants": ["user_id_1", "user_id_2"],
  "messages": [
    { "messageId": "msg_id", "text": "Hello!", "type": "text", "timestamp": 1713094800000 }
  ]
}
```
...
The app uses WebSocket for real-time call signaling on `wss://hostname:3001`
...
// Initiate Call
{
  ...
  from: string,
  to: string,
  payload: { caller: CallPeer, callId: string }
}
...
// Send Translation (Sign Language)
{
  type: 'sign-translation',
  callId: string,
  from: string,
  to: string,
  payload: { text, timestamp, shouldSpeak }
}

// Send Transcript (Speech-to-Text)
{
...
interface AppContextType {
  // User Management
  user: User | null;
  login: (user: { username: string, isDeaf: boolean }) => void;
  logout: () => void;
  ...
  // Messages
  translationMessages: TranslationMessage[];   // Sign language recognition results
  transcriptMessages: TranslationMessage[];    // Speech-to-text transcripts
...
- `"Invalid username or password"` - Wrong password for existing user
- `"username must be at least 3 characters long"` - Username validation failed
- `"password must be at least 6 characters long"` - Password validation failed
- `"User not found"` - User ID doesn't exist
- `"This username is already registered"` - Duplicate username
